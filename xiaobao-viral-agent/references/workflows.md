...
1. **先问要不要包装**：交付 `video_url`，问「还需要模版包装成片吗？」  
   - 不要 → 结束，禁止 create  
   - 要 / 「全自动」→ 继续下面三项（全自动可跳过细节）
2. **再问包装细节**（用户说「全自动 / 你来安排」可跳过）：  
   - **模版**：自己选（列 2～4 个中文风格） / 系统自动选  
   - **补充素材**：只要主视频 / 再传视频或图片  
   - **封面与标题**：先给草案确认 / 全权智能安排  

未确认「要包装」前禁止 `create-task`。不要默认沿用历史模版闷头出片。

### 自动处理（确认之后）

1. 判定 `realMan`
2. `list-templates --scene realMan` → 按闸门结果选模版（自选或自动）
3. 若无成片口播视频：内部 TTS（及可选数字人合成）得到口播视频 URL（此步可在闸门前完成）
4. 素材/字幕：按确认结果写入；可 `recognize`
